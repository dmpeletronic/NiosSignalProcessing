...
################################################################################
set pin_file_name "sdram_phy_ddr_pins.tcl"
...
# instantiations.
################################################################################
proc add_requirements_for_instance {corename instance_name t_name sdram_phy_use_high_performance_timing} {
	upvar 1 $t_name t
...
# Create the PLL input clock and derive clocks on the PLL outputs
	...
	if {$ck_pll_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "sdram_phy_ddr_timing.sdc: $msg"
		}
		post_message -type warning "sdram_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(ck_p)]"
	} else {
		...
		if {$pll_ref_clk_id != -1} {
			...
			post_message -type info "sdram_phy_ddr_timing.sdc: Could not find PLL clocks for $ck_pll_clock. Creating PLL base clocks"
			# Attempt to recover
...
# Find the TimeQuest name for the resync clock. If it is not found, create one.
	...
	if {$resync_pll_ref_clk_id != -1} {
		...
		post_message -type warning "sdram_phy_ddr_timing.sdc: Failed to find PLL input clock pin driving $resync_clock"
	}
...
################################################################################
	...
	if {$dqs_pll_clock_id == -1 || $dq_pll_clock_id == -1} {
		...
		post_message -type warning "sdram_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join [get_all_dqs_pins $pins(dqsgroup)]]"
	} else {
...
#False path the control_wlat_r values from the controller, as these will be static when being used :
...
################################################################################
	...
	if {$ac_pll_clock_id == -1} {
		...
		post_message -type warning "sdram_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(addrcmd)]"
	} else {
...
foreach inst $instance_list {
	...
	add_requirements_for_instance $corename $inst t $sdram_phy_use_high_performance_timing
