...
void AquisicaoRedeTask(void *task_data){
	OS_FLAGS value;
	INT8U error_code;
	float temp;
	int i,j;
	...
	while(1){
		...
		printf("\n[Aquisição de dados] Aguarda evento");
	    value = OSFlagPend(EventoSocket, 0x0001, OS_FLAG_WAIT_SET_ANY+OS_FLAG_CONSUME, 0, &error_code);
	...
	//	PERF_BEGIN (PERFORMANCE_COUNTER_BASE, 1);
		...
		for(i=0; i<256; i++){
			    ponto.full = ((long)(pacote.Dados.Aquis[i])-2000);
			  //ponto.full = (long)((float)(pacote.Dados.Aquis[i]/*-1250*/)/10);
			 //   printf("%d = %f --- ", ponto.full, FIXED2_16toReal(ponto));
	...
	/*	PERF_STOP_MEASURING(PERFORMANCE_COUNTER_BASE);
			    perf_print_formatted_report((void *)PERFORMANCE_COUNTER_BASE,
				   ALT_CPU_FREQ, 3,  "Envia dados", "ExecutaFFT", "Leitura dos dados");
	*/	OSTimeDlyHMSM(0,0,1,0);
...
void CopiaResultadoAoBuffer(FIXED2_16 *real, FIXED2_16 *imag, int size, unsigned char * buffer){
		int i,j;
		unsigned char rMSB, rLSB, iMSB, iLSB;

		j=0;
		for(i=0; i<4*size; i+=4){
			//imprimeFIXED2_16("real", real[j]);
			//imprimeFIXED2_16("imag", imag[j]);
			//printf("r: %f ", FIXED2_16toReal(real[j]));
			//printf("i: %f\n", FIXED2_16toReal(imag[j]));
			rMSB = (signed char)(real[j].full>>10)&0xFF;
			rLSB = (unsigned char)((real[j].full>>2)&0xFF);
			iMSB = (signed char)(imag[j].full>>10)&0xFF;
			iLSB = (unsigned char)((imag[j].full>>2)&0xFF);
			buffer[i] = rMSB;
			buffer[i+1] = rLSB;
			buffer[i+2] = iMSB;
			buffer[i+3] = iLSB;
		...
		while(1){
			...
			switch(estado_socket_sender_result){
				...
				case sending:
					...
					if(enviados>0){
						printf("\n[SocketSenderResultTask] Enviados %d dados ao computador.", enviados);
						estado_socket_sender_result = waiting;
					}else
					{
					...
					case receiving:
							...
							if(recebidos>0){
								   ...
								   CopiaDadosAoPacote(buffer_rx, &pacote);
									...
									OSFlagPost(EventoSocket, 0x0001, OS_FLAG_SET, &error_code);
