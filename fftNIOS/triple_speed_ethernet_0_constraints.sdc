...
# set_global_assignment -name SDC_FILE "triple_speed_ethernet_0"_constraints.sdc
#
# Generated on Fri Jan 27 00:41:54 BRST 2012
#
#####################################################################################
...
# General Option
set IS_SOPC 1
set VARIATION_NAME "triple_speed_ethernet_0"
        ...
        if {[ expr [regexp "altera_tse" $name] == 1]} {
            ...
            if {[ expr [regexp "altlvds_rx_component" $name] || [regexp $REF_CLK $name]]} {
                if { [ expr ($IS_PCS == 1) && ($IS_PMA == 1) && ($TRANSCEIVER_TYPE == 1)] } {
                    set_clock_groups -exclusive -group [get_clocks "altera_tse_${REF_CLK}* *|altlvds_rx_component|auto_generated|rx[0]|clk0 *|altlvds_rx_component|auto_generated|pll|clk[0]"]
                } else {
